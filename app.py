...
with st.sidebar:
    ...
    st.markdown("Upload multiple PDF documents and index them for grounded QA.")
        ...
        if not uploaded_files:
            ...
            with st.spinner("Extracting text and generating vector embeddings..."):
                for pdf_file in uploaded_files:
                    # Use getvalue() to safely read raw bytes without losing stream position
    ...
    if vector_store.get_count() == 0:
        ...
        with st.chat_message("assistant"):
            with st.spinner("Searching document context and generating response..."):
                retrieved_chunks = retriever.retrieve(query=prompt, top_k=top_k)
                answer, citations = generator.generate_answer(query=prompt, context_chunks=retrieved_chunks)
